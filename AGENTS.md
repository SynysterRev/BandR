...
- Un musicien consulte et gère son propre profil via `GET` et `PATCH /api/Musicians/me`. La désactivation du compte est faite via `DELETE /api/Account/me` : elle désactive les annonces et les conversations liées, tout en conservant les données. Ne pas réintroduire une mutation personnelle basée sur `/{id}` : réserver les futures mutations `/{id}` à une politique d'administration explicite.
- La désactivation de compte est transactionnelle : conserver dans une même transaction la mise à jour Identity, la désactivation des annonces/conversations et la révocation des refresh tokens.
